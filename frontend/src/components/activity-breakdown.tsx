    ...
    <div className="bg-white rounded-lg border border-stone-200 p-4 space-y-4">
        ...
        {dimensionConfig.map(({ key, label, max }) => {
          ...
          return (
            <div key={key} className="flex items-center gap-2">
              <span className="text-xs text-stone-500 w-20 text-right">{label}</span>
              <div className="flex-1 flex gap-0.5">
                {Array.from({ length: max }).map((_, i) => (
                  <div
                    key={i}
                    className={`h-2.5 flex-1 rounded-sm ${
                      i < value ? "bg-emerald-500" : "bg-stone-200"
                    }`}
                  />
                ))}
              </div>
              <span className="text-xs font-mono-data text-stone-400 w-8">
                {value}/{max}
              </span>
            </div>
          );
      ...
      <div className="pt-3 border-t border-stone-100 space-y-2">
        ...
        {stats.platforms.length > 0 && (
          <div className="flex items-center gap-1.5 flex-wrap">
            {stats.platforms.map((p) => (
              <span
                key={p}
                className="inline-flex items-center gap-1 px-2 py-0.5 text-xs rounded-full bg-stone-100 text-stone-600"
              >
        ...
        <p className="text-xs text-stone-400">
          <span className="font-mono-data text-stone-600">{stats.interaction_count}</span> total interactions
        </p>
      </div>
          ...
          <div className="flex items-end gap-1 h-16">
            {monthly_trend.map((m) => (
              <div key={m.month} className="flex-1 flex flex-col items-center gap-1">
                ...
                <span className="text-[10px] text-stone-400 font-mono-data">{m.count}</span>
              </div>
            ))}
          </div>
...
export function ActivityBreakdownSkeleton() {
  return (
    <div className="bg-white rounded-lg border border-stone-200 p-4 space-y-4 animate-pulse">
      <div className="h-3 w-32 bg-stone-200 rounded" />
      <div className="space-y-2">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="flex items-center gap-2">
            <div className="h-3 w-20 bg-stone-100 rounded" />
            <div className="flex-1 h-2.5 bg-stone-100 rounded" />
          </div>
        ))}
      </div>
