...
/* ── Common Telegram Groups Card ── */

function CommonGroupsCard({ contactId, hasTelegram }: { contactId: string; hasTelegram: boolean }) {
  const [groups, setGroups] = useState<{ id: number; title: string; username?: string; link?: string; participants_count?: number }[]>([]);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    client.GET("/api/v1/contacts/{contact_id}/telegram/common-groups", {
      params: { path: { contact_id: contactId } },
    }).then(({ data }) => {
      setGroups((data?.data as typeof groups) ?? []);
    ...
    <div className="bg-white rounded-xl border border-stone-200 p-5">
      ...
      {loading ? (
        <div className="space-y-2 animate-pulse">
          {[1, 2].map((i) => <div key={i} className="h-4 bg-stone-100 rounded" />)}
        </div>
      ) : (
          ...
          {groups.map((g) => (
            <div key={g.id} className="flex items-center justify-between text-sm">
              <div className="flex items-center gap-2 min-w-0">
                ...
                {g.link ? (
                  <a href={g.link} target="_blank" rel="noopener noreferrer" className="text-stone-700 hover:text-sky-600 truncate">
                    {g.title}
                  </a>
                ) : (
              ...
              {g.participants_count != null && (
                <span className="text-[11px] text-stone-400 shrink-0 ml-2">{g.participants_count} members</span>
              )}
            </div>
          ))}
        </div>
...
function RelatedContactsCard({ contactId }: { contactId: string }) {
  const { data } = useQuery({
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:order-1 space-y-6">
            ...
            {/* Common Telegram Groups */}
            <CommonGroupsCard contactId={id} hasTelegram={Boolean(contact.telegram_username)} />
